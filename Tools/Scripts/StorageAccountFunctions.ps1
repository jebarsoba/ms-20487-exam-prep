Function Verify-StorageAccount
{ 
    $storageAccounts = Get-AzureStorageAccount 

    if ($storageAccounts -eq $null -or $storageAccounts.Length -eq 0)
    {        
        ...
        $storageAccountName = "bycl" + $date + "" + $initials        
        $region = Get-Region
        Write-Host Creating a Windows Azure Storage account. This may take several minutes to complete. -ForegroundColor Cyan
        Write-Host 
        $storageAccount = New-AzureStorageAccount -StorageAccountName  $storageAccountName -Location $region
        $storageAccounts = Get-AzureStorageAccount 
    }

	$storageAccount = $storageAccounts[0]
    
    $storageAccountKey = Get-AzureStorageKey -StorageAccountName $storageAccount.StorageAccountName

    $props = @{
        AccountInfo = $storageAccount
        AccountKey = $storageAccountKey.Primary
    }

    $storageAccountInformation = new-object psobject -Property $props
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUSBb3SkyuEF9r0dEKZYOmFZO7
# SySgghWCMIIEwzCCA6ugAwIBAgITMwAAADPlJ4ajDkoqgAAAAAAAMzANBgkqhkiG
...
# HAYKKwYBBAGCNwIBCzEOMAwGCisGAQQBgjcCARUwIwYJKoZIhvcNAQkEMRYEFEJQ
# 452fC+H7qT/f/ATcd42PTjvVMF4GCisGAQQBgjcCAQwxUDBOoCaAJABNAGkAYwBy
# AG8AcwBvAGYAdAAgAEwAZQBhAHIAbgBpAG4AZ6EkgCJodHRwOi8vd3d3Lm1pY3Jv
# c29mdC5jb20vbGVhcm5pbmcgMA0GCSqGSIb3DQEBAQUABIIBALGtAYAbQIYENb9d
# kOfbSM+UdV0re/fQgnLLtmm33vyaDcoAWiD6jRpv3EniCSf0OWMB1l99AvG3PxOS
# 5pyzHNNzyVWY806ttUVZGWp/jtRYzRZ0kde+/QUr2UBTnLdj+bPyNbao0iDdxT7q
# qQnTHjU/Hsvo77WbM0OjRsp+Gmt1ae53mUHjqecZYW3RERmiuTBnjhA9LekQEwXA
# E4p6rWQCGxRmCNLHPW3cA0vl9Y1bfVuSok352Cb4E8c1lr26feF7YRk6BVjxac1E
# 4wgeeoVu0RQVroej5PIzzx2uHn6xr4dB7g1McYvvYyIOJWj1N5f1pXSB80/xg674
# /njnA56hggIoMIICJAYJKoZIhvcNAQkGMYICFTCCAhECAQEwgY4wdzELMAkGA1UE
...
# MjMyMDE5MTJaMCMGCSqGSIb3DQEJBDEWBBTRYifmNZ5HA0drkWnd2l2bQABAizAN
# BgkqhkiG9w0BAQUFAASCAQA3uF3Iz/FV+pIwAiTMITxY7Ywx6fypYGYglsZ0OMZt
# Q24/3bhqAMUUeaww/d4VPkrdjuoQvk9NsA8O/zsXo2Rb/Y96yxndHIYXKfhQBJxO
# AaWNrAGbOcZItS+ZkBCy4B9pJwUnzvxP1YWYPAilBVgPP30QAZD1yAMEjcMrHkKh
# UGv6uaX8RBr2BXTAuC64zJl9qZfYjDlfxzxd4e/fEtKKJCTIShADqBoRvgPfapav
# GBPrXy3F08kq4Yw7ILi5JM70+LkSRe15ycbJiARNIjxNmDJN3igR9jGwxLmCxWWR
# DU7AaF3i8snyflOw3Kr1T3Kh+aS9LwpR8Bc3reEaPY3y
