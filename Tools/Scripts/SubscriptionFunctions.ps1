Function Set-DefaultAzureSubscription
{
    ...
    # Remove previous .publishsettings files
    Join-Path $path *.publishsettings | Remove-Item
    # Clear previous subscriptions
    Get-AzureSubscription -ErrorAction SilentlyContinue | Remove-AzureSubscription -force -WarningAction Ignore

    Write-Host "Press Enter to open Windows Azure subscription profile download"
    Write-Host "After the browser opens, sign in if required, and then save the subscription file to " -nonewline
    write-host $path -ForegroundColor Yellow
    Write-Host "After the file has been saved, return to this PowerShell window, and press Enter to continue"
    Read-Host    
    Get-AzurePublishSettingsFile
    Read-Host 
    $publishFiles = Join-Path $path *.publishsettings | Get-Item

    if ($publishFiles.length -eq 0) {
        write-host "No .publishsettings file found. Please run the script again and save the file to the correct location"   
    }
    elseif ($publishFiles.length -lt 1) {
        write-host "Found too many .publishsettings files. Please delete old publish settings files and leave only one remaining"   
    } else {
        Import-AzurePublishSettingsFile $publishFiles[0] | out-null

        $subscriptions = Get-AzureSubscription    
        if ($subscriptions.length -eq 0)    {
            Write-Host "No subscriptions found in the .publishsettings file, please call the instructor"
        }
        elseif ($subscriptions.length -gt 1)
        {   
            # Need to set the default profile 
            Write-Host "The following subscriptions were found:"
        
            foreach($item in $subscriptions) {Write-Host $item.SubscriptionName}        
            $subscriptionName = Read-Host -Prompt "Please type the name of the subscription you want to use"        
            #$subscriptionName = "MSDN Account"
            # Set the default subscription to the selected one
            Set-AzureSubscription -default $subscriptionName       
        }
    }
}
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUZ+yl9R9RW+JaXmBp6kAQbcs3
# U/qgghWCMIIEwzCCA6ugAwIBAgITMwAAACs5MkjBsslI8wAAAAAAKzANBgkqhkiG
...
# HAYKKwYBBAGCNwIBCzEOMAwGCisGAQQBgjcCARUwIwYJKoZIhvcNAQkEMRYEFKax
# dRDiZ9pBKlnSH/mrkJOmx2pmMF4GCisGAQQBgjcCAQwxUDBOoCaAJABNAGkAYwBy
# AG8AcwBvAGYAdAAgAEwAZQBhAHIAbgBpAG4AZ6EkgCJodHRwOi8vd3d3Lm1pY3Jv
# c29mdC5jb20vbGVhcm5pbmcgMA0GCSqGSIb3DQEBAQUABIIBAFJt55NP4Q6W/zBG
# 1rdyZuXT80xzEC6a52NJuhjoNuPsD+uf0A4Pqn6YFZ/gjAllXv1uRXyUuPYsm12W
# KM8AoLd6K61lG1gDASpMIujoQpTpukFHdP7/8L/ERmWNDTXIPz4K+n4cbJcF1xdg
# 4kD2QEyjNEQOvf3L++X2IaAyhJr0CsPVbQvZ8yb9Rqodv1lhR2GKJudYFdcmDYtU
# i60Mjl32tpipq1x9/chNTXcuo7wd7HUpYm9z0j+Xkc5uNvch0HWw6PGUNGcXwmxT
# iD1hc59wpiWGMvw4uUDS5YIz/qbJcJN0VDGWPowirCeVWxldVzpng3z5f6nRYWFZ
# TkqmBPqhggIoMIICJAYJKoZIhvcNAQkGMYICFTCCAhECAQEwgY4wdzELMAkGA1UE
...
# MTUxNzI4MDNaMCMGCSqGSIb3DQEJBDEWBBQVLGLFSn2QegKmEM7SgJQ8E3sq+jAN
# BgkqhkiG9w0BAQUFAASCAQCZjFb7M1h0ZDvNj9ryjTn+kxsZCYD3Xh6J32IYIVGR
# wAWpFUXWHrAe4exrIuH+0jbOSv8rMclGWywWMzQdVt1yTnH+1nxcoirb47bFmOiD
# +G/WPXPjEsaUeh+lNIjk53yEyEjjLJT3cdIXFYyN+01UPzIgHSuFnDweyOl7JISf
# DlLpkhbLJ26qQZRQOUKPeD1d4sH2dVMpwqiknSY8Dt9UbXTYaZiJaqJA79rOpuHT
# wF+tj0cxZbkqI9Qn32lt42iIHCy98IL11BZaibA0dnGqNmYcY5HzOQLSyuwaYjPi
# TcKAhh7WsQ7Ubl9T1A1UKp/zwGBwUcuyc0ZZg9B6pg6+
